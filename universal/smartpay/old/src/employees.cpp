    ...
    ui(new Ui::Employees)
{
    ...
    if (!DataPublics::checkUserAccess("View Finance Data", db)) {
        hideFinance();
    }
}
...
void Employees::reloadSaccos()
{
    ...
    ui->SaccoTree->setQuery("SELECT * FROM vw_employee_sacco_membership WHERE EmployeeID = '" + currentID + "'", db, "Sacco Membership for " + currentName, false);
    saccoModel = ui->SaccoTree->myModel;
...
{
    MakeSaccoPayment *newSaccoPayment = new MakeSaccoPayment(this);
    if (newSaccoPayment->exec()) {
