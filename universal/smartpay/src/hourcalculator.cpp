...
void HourCalculator::CalculateAttendanceForDay(QString date, QString employeeID, QSqlDatabase db)
{
	...
	if (qu2.size() > 0) {
		QString clockIn = shiftStartTime;
		QString clockOut = shiftEndTime;
		while (qu2.next()) {
		...
		double penaltyMins = lateMinutes + earlyExitMinutes;
		...
		db.exec("INSERT INTO employees_attendance_ex (EmployeeID, Date, ShiftID, "
			...
			clockOut + "', '" + QString::number(penaltyMins) + "')");
	}
}
