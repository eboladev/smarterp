	...
	ui(new Ui::ReportViewer)
{
	...
	//ui->widget = new QPrintPreviewWidget(printer, this);
...
void ReportViewer::loadReport(QString reportName, QString param_where)
{
	...
	QSqlQuery rq = db.exec("SELECT * FROM report WHERE report_name= '" + reportName + "' ORDER BY report_grade DESC LIMIT 0,1");
