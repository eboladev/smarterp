...
{
    ui->lstAttendance->setVisible(false);
    ...
    QSqlRelationalTableModel *attendanceModel = new QSqlRelationalTableModel(this, db);
    attendanceModel->setTable("employees_attendance");
    attendanceModel->setFilter(" Date = '" + DataPublics::mysqlDate(ui->dtpPast->date()) + "'");
    attendanceModel->setEditStrategy(QSqlRelationalTableModel::OnRowChange);
    attendanceModel->setRelation(1, QSqlRelation("vw_employeenames_ex", "employeeID", "Name"));
    attendanceModel->select();
    ui->tblAttendance->setModel(attendanceModel);
    ui->tblAttendance->setColumnHidden(0, true);
    ui->tblAttendance->setColumnHidden(1, true);
...
void Attendance::on_dtpPast_editingFinished()
{
