...
{
	ui->treeView->setQuery("SELECT * FROM `vw_timetable` WHERE `Date` BETWEEN '" + Publics::getDateString(ui->StartDate->date()) + "' AND '" + Publics::getDateString(ui->EndDate->date()) + "' ORDER BY `Date`", db, "Attendance Timetable", true);
