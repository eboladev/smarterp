...
#include "ui_advanceswizard.h"
#include "datapublics.h"

AdvancesWizard::AdvancesWizard(QWidget *parent, QSqlDatabase database) :
    QWizard(parent),
    ui(new Ui::AdvancesWizard)
{
    ui->setupUi(this);
    db = database;
...
AdvancesWizard::~AdvancesWizard()
{
    delete ui;
}

void AdvancesWizard::on_commandLinkButton_clicked()
{
    QString quError = "";
    for (int i = 0; i < ui->tblAdvances->rowCount(); i++) {
        ...
        QSqlQuery qu =  db.exec("INSERT INTO employee_advances (EmployeeID, AdvanceDate, Amount) VALUES ('"
                + employeeID + "', '"
...
void AdvancesWizard::on_lstEmployees_itemChanged(QTreeWidgetItem *item, int column)
{
    Q_UNUSED(column);
