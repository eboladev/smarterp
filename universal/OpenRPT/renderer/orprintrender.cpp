...
bool ORPrintRender::setupOcPrinter(ORODocument *pDocument, QPrinter *pPrinter)
{
	if(pDocument == 0 || pPrinter == 0)
		return false;
	...
	pPrinter->setPageOrder(QPrinter::FirstPageFirst);
	QSizeF size(8.5, 5.5);
	pPrinter->setPaperSize(size, QPrinter::Inch);
	return true;
}

bool ORPrintRender::renderOC(ORODocument *pDocument)
{
	...
	QSizeF size(8.5, 5.5);
	_printer->setPaperSize(size, QPrinter::Inch);

	bool deleteWhenComplete = false;
	bool endWhenComplete = true;
	...
	QSize margins(_printer->paperRect().left() - _printer->pageRect().left(),
		      _printer->paperRect().top() - _printer->pageRect().top());
	renderPage(pDocument, 0, _painter, xDpi, yDpi, margins, _printer->resolution());
