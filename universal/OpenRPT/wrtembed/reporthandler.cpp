    ...
    : QObject(parent)
{
    ...
    helpAboutQAction = new QAction (tr("About Qt"), this);
...
*/
    ...
    connect (helpAboutQAction, SIGNAL(triggered()), SLOT(aboutQt()));

    dbConnect();

    fileSaveAction->setVisible(false);
    fileSaveAsAction->setVisible(false);
    fileOpenAction->setVisible(false);

    fileExitAction->setVisible(false);
...
QAction * ReportHandler::populateMenuBar(QMenuBar * menubar, QAction * exitAction)
{
    ...
    mFile->addAction(dbLoadAction);
    mFile->addAction(dbSaveAction);
    ...
    mHelp->addAction(helpAboutAction);
    mHelp->addAction(helpAboutQAction);
...
DocumentWindow * ReportHandler::fileOpen(const QDomDocument & doc)
{
    ...
    connect(dv, SIGNAL(m_changed(QString)), SIGNAL(childChanged(QString)));
...
void ReportHandler::insertFieldFromList(QString fieldName)
{
    //itemField();
    DocumentWindow * gw = activeDocumentWindow();
    if(gw)
    {
        ...
        if(list.count() > 0)
        {
            ...
            for(int i = 0; i < list.count(); i++)
            {
                item = list.at(i);
                int rtti = item->type();
                if(rtti == ORGraphicsFieldItem::Type)
                {
                    ORGraphicsFieldItem * ent = (ORGraphicsFieldItem*)item;
                    ent->setQuery("Master");
                    ent->setColumn(fieldName);
                }
            }
        }
    }
...
// add in support for a pointer/grabber/none option
...
void ReportHandler::dbConnect() {
    ...
    bool dbConnected = _db.isOpen();//(dbConnectAction->text() == _strMenuDisconnect);
    ...
    if(!dbConnected) {
        ...
        QSqlDatabase db ;
        db = _db;
        OpenRPT::databaseURL = "QMYSQL://" + db.hostName() + ":" + QString::number(db.port()) + "/" + db.databaseName();
        emit dbOpenClosed();
    } else {
        emit dbOpenClosed();
    }
...
void ReportHandler::dbLoadDoc() {
    //QSqlDatabase db = _db;//QSqlDatabase::database(QSqlDatabase::defaultConnection, true);
    if (_db.isOpen()) { //if(db.isValid()) {
        ...
        if(rptDiag.exec() == QDialog::Accepted) {
            ...
            _serverName = _db.hostName();
            _userName = _db.userName();
            _password = _db.password();
            _reportName = rptDiag.getName();
            emit aReportOpen();
            ...
            if(doc.setContent(rptDiag.getSource(),&errMsg,&errLine,&errCol)) {
                ...
                if(rw)
                {
                    ...
                    connect(rw, SIGNAL(m_changed(QString)), SIGNAL(childChanged(QString)));
                }
            } else {
...
DocumentWindow * ReportHandler::activeDocumentWindow()
{
     ...
     */
        ...
        if (i >= 0 && list.at(i)->inherits("DocumentWindow"))
            if (qobject_cast<DocumentWindow*>(list.at(i))) {
                DocumentWindow *doc = qobject_cast<DocumentWindow*>(list.at(i));
                return doc;
            }
        return 0;
    }
...
void ReportHandler::dbMenuAboutToShow() {
    dbConnectAction->setVisible(false);
    //QSqlDatabase db = QSqlDatabase::database(QSqlDatabase::defaultConnection, true);
...
void ReportHandler::sReportsChanged(int pReportid, bool pLocaleUpdate)
{
    ...
    //emit childChanged();
    //qDebug() << "Changed" << gw->_handler->_reportName;
    emit childChanged(gw->_handler->_reportName);
    //emit childChanged("");
}

void ReportHandler::onWinChangedEx(QMdiSubWindow *w) {
...
{
#if defined Q_WS_WIN
    return QObject::tr("ReportMaster for Windows");
#elif defined Q_WS_X11
    return QObject::tr("ReportMaster for Linux");
#elif defined Q_WS_MAC
    return QObject::tr("ReportMaster for OS X");
#else
    return QObject::tr("ReportMaster Generator");
#endif
}

QString ReportHandler::copyright()
{
    return QObject::tr("Copyright (c) 2011-2012, J.W. Joshua\n<joejoshw@gmail.com>.");
}
...
QString ReportHandler::getIconName(const QString& pDefaultName)
{
    ...
    // Get toolbar iconsize from options
...
void ReportHandler::loadMemDB(const QString &filename, const QDomNode &it)
{
    ...
    MemDbLoader mdb;
    bool ok = mdb.load(_databaseElt, _db);//QSqlDatabase::database());
    if(!ok)
        QMessageBox::warning(0, filename, mdb.lastError());
}

void ReportHandler::aboutQt()
{
    QMessageBox::aboutQt(0, "About Qt");
}

void ReportHandler::about()
{
    QMessageBox::about(0, "About ReportMaster Generator", "(c) Joseph W Joshua\n2012\njoejoshw@gmail.com");
}
