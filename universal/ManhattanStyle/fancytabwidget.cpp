	...
	: QWidget(parent)
{
	...
	connect (m_tabBar, SIGNAL(currentChanged(int)), this, SLOT(tabChanged()));
...
void FancyTabWidget::tabChanged()
{
	QString tabName = m_tabBar->tabText(m_tabBar->currentIndex());
	emit currentTextChanged(tabName);
}
...
void FancyTabWidget::insertTab(QWidget *tab)
{
	int index  = m_tabBar->count();
	insertTab(index, tab, tab->windowIcon(), tab->windowTitle());
}

void FancyTabWidget::insertTab(QWidget *tan, QIcon &icon)
{
	int index  = m_tabBar->count();
	insertTab(index, tan, icon, tan->windowTitle());
}
