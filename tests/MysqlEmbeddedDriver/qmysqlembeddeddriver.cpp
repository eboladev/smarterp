...
{
public:
    QMYSQLEmbeddedDriverPrivate() : mysql(0),
...
class QMYSQLEmbeddedResultPrivate : public QObject
{
    Q_OBJECT
public:
    QMYSQLEmbeddedResultPrivate(const QMYSQLEmbeddedDriver* dp, const QMYSQLEmbeddedResult* d) : driver(dp), result(0), q(d),
    ...
    const QMYSQLEmbeddedDriver* driver;
    MYSQL_RES *result;
    MYSQL_ROW row;
    const QMYSQLEmbeddedResult* q;
...
: QSqlResult(db)
{
    d = new QMYSQLEmbeddedResultPrivate(db, this);
}
...
bool QMYSQLEmbeddedResult::nextResult()
{
    if(!d->driver)
        return false;
#if MYSQL_VERSION_ID >= 40100
    ...
    if (status > 0) {
        setLastError(qMakeError(QCoreApplication::translate("QMYSQLEmbeddedResult", "Unable to execute next query"),
                     QSqlError::StatementError, d->driver->d));
        return false;
    } else if (status == -1) {
    ...
    if (!d->result && numFields > 0) {
        setLastError(qMakeError(QCoreApplication::translate("QMYSQLEmbeddedResult", "Unable to store next result"),
                     QSqlError::StatementError, d->driver->d));
        return false;
    }
...
void QMYSQLEmbeddedResult::virtual_hook(int id, void *data)
{
    switch (id) {
...
//                               optionFlags))
 ...
 //   "--skip-innodb",
  //  "--console",
    ...
    qDebug() << "Loading embedded server...";
    mysql_library_init(num_elements, server_options, server_groups);
    d->mysql = mysql_init(NULL);
   // mysql_options(d->mysql, MYSQL_READ_DEFAULT_GROUP, db.toLocal8Bit().constData());
    mysql_options(d->mysql, MYSQL_OPT_USE_EMBEDDED_CONNECTION, NULL);

    if (mysql_real_connect(d->mysql, NULL, NULL, NULL, db.toLocal8Bit().constData(), 0, NULL, 0))
    {
...
QString QMYSQLEmbeddedDriver::escapeIdentifier(const QString &identifier, IdentifierType) const
{
...
bool QMYSQLEmbeddedDriver::isIdentifierEscapedImplementation(const QString &identifier, IdentifierType type) const
{
    Q_UNUSED(type);
...
#include "qmysqlembeddeddriver.moc"
