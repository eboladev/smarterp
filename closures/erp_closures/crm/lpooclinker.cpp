...
void LPOOcLinker::reloadOCs()
{
    ...
    model->setQuery("SELECT entry_id as 'Order No', order_date as 'Order Date',"
                    "order_qty as 'Order Qty', lpo_comment as 'Comment' FROM oc WHERE lpo_id = '"
                    + m_orderID + "' ", db);
...
void LPOOcLinker::on_cmdAddOc_clicked()
{
    ...
    if (ocNo.length() > 0) {
        db.exec("UPDATE oc SET lpo_id = '" + m_orderID + "' WHERE entry_id = '" + ocNo + "'");
        reloadOCs();
    }
}
...
void LPOOcLinker::on_cmdRemoveSelected_clicked()
{
    ...
    if (ocNo.length() > 0) {
        db.exec("UPDATE oc SET lpo_id = '' WHERE entry_id = '" + ocNo + "'");
        reloadOCs();
    }
}
