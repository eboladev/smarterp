    ...
    ui(new Ui::NewLPO)
{
    ...
    QSqlQuery qu = db.exec("SELECT * FROM products WHERE Deletd = 'No' AND customer_id = '" + cID + "'");
...
void NewLPO::setProducts()
{
    for (int i = 0; i < prodIDs.count(); i++) {
        ...
        productName = DataPublics::getDbValue("SELECT * FROM products WHERE product_id = '" + currentID + "'", db, "product_name").toString();
        price = DataPublics::getDbValue("SELECT * FROM products WHERE product_id = '" + currentID + "'", db, "total_price").toString();
...
{
    QString lpoQuery = "INSERT INTO purchase_orders (CustomerID, Date, SubTotal, VAT, OrderTotal, Comment)"
            " VALUES ('" + customerID + "', '"
